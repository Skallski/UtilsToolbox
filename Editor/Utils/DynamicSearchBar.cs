...
namespace UtilsToolbox.Editor.Utils
{
    /// <summary>
    ...
    public sealed class DynamicSearchBar
    {
        ...
        private void ShowSearchResults()
        {
            ...
            IEnumerable<SearchBarItem> items = _items
                .Where(item => item.Content.text.StartsWith(_searchQuery, StringComparison.OrdinalIgnoreCase))
                .Where(item => GUILayout.Button(item.Content));
            
            foreach (SearchBarItem item in items)
            {
